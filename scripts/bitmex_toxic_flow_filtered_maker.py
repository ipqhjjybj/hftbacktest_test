...
TOXIC_FLOW_THRESHOLD = 0.35
TOXIC_MOMENTUM_BPS = 0.45
TOXIC_MICROPRICE_BPS = 0.30
...
@njit
def record_signal(hbt, signal_ts, signal_bid, signal_ask, signal_bid_qty, signal_ask_qty, write_idx, count):
    depth = hbt.depth(0)
...
@njit
def side_toxic_score(side, flow_sig, momentum_bps, micro_bps):
    score = 0.0
    if side > 0:
        score += max(0.0, -flow_sig / max(TOXIC_FLOW_THRESHOLD, 1e-9))
        score += max(0.0, -momentum_bps / max(TOXIC_MOMENTUM_BPS, 1e-9))
        score += max(0.0, -micro_bps / max(TOXIC_MICROPRICE_BPS, 1e-9))
    else:
        score += max(0.0, flow_sig / max(TOXIC_FLOW_THRESHOLD, 1e-9))
        score += max(0.0, momentum_bps / max(TOXIC_MOMENTUM_BPS, 1e-9))
        score += max(0.0, micro_bps / max(TOXIC_MICROPRICE_BPS, 1e-9))
...
@njit
def hard_toxic(side, flow_sig, momentum_bps, micro_bps):
    if side > 0:
        return (
            flow_sig <= -TOXIC_FLOW_THRESHOLD
            or momentum_bps <= -TOXIC_MOMENTUM_BPS
            or micro_bps <= -TOXIC_MICROPRICE_BPS
        )
    return (
        flow_sig >= TOXIC_FLOW_THRESHOLD
        or momentum_bps >= TOXIC_MOMENTUM_BPS
...
@njit
def target_price(side, depth, pos, flow_sig, momentum_bps, micro_bps, vol_bps):
    mid = (depth.best_bid + depth.best_ask) / 2.0
    inv = inventory_ratio(pos)
    min_half_spread_bps = MIN_HALF_SPREAD_TICKS * BITMEX_TICK_SIZE / mid * 10_000.0
    toxic_score = side_toxic_score(side, flow_sig, momentum_bps, micro_bps)
    half_spread = max(
        BASE_HALF_SPREAD_BPS
        + VOL_SPREAD_MULTIPLIER * vol_bps
        + TOXIC_WIDEN_MULTIPLIER * min(toxic_score, 3.0)
...
@njit
def should_quote(side, pos, flow_sig, momentum_bps, micro_bps):
    ...
    if side < 0 and pos - ORDER_QTY < -MAX_POSITION_CONTRACTS:
        return False, 2
    if hard_toxic(side, flow_sig, momentum_bps, micro_bps) and not is_reduce_side(side, pos):
        return False, 3
    return True, 0
...
def manage_side(hbt, side, order_id, target_px, should_place, inflight_until, next_rest_allowed_ts, live_since, metrics):
    ...
    toxic_cancel_metric = 12 if side > 0 else 13
...
def run_strategy(hbt, recorder, metrics, end_close_ts):
    ...
    while hbt.elapse(ORDER_UPDATE_INTERVAL_NS) == 0:
        ...
        depth = hbt.depth(0)
        if depth.best_bid <= 0 or depth.best_ask <= 0:
            if hbt.orders(0).get(bid_id) is not None:
                hbt.cancel(0, bid_id, False)
            if hbt.orders(0).get(ask_id) is not None:
                hbt.cancel(0, ask_id, False)
        else:
            pos = hbt.position(0)
            ...
            bid_ok, bid_reason = should_quote(1, pos, flow_sig, momentum_bps, micro_bps)
            ask_ok, ask_reason = should_quote(-1, pos, flow_sig, momentum_bps, micro_bps)
            if bid_reason == 3:
                metrics[20] += 1
            if ask_reason == 3:
                metrics[21] += 1

            bid_px = target_price(1, depth, pos, flow_sig, momentum_bps, micro_bps, vol_bps)
            ...
            ask_px = target_price(-1, depth, pos, flow_sig, momentum_bps, micro_bps, vol_bps)
...
def run_backtest(bitmex_npz: Path, yyyymmdd: str) -> Path:
    ...
    tag = RESULT_TAG or f"toxic_flow_filtered_{EXCHANGE_MODEL}"
    out = RESULT_DIR / f"bitmex_xbtusdt_toxic_flow_filtered_maker_{tag}_{yyyymmdd}.npz"
...
def write_summary(result_npz: Path, yyyymmdd: str) -> dict:
    ...
    summary = {
        ...
        "strategy": "toxic_flow_filtered_maker",
        ...
        "toxic_flow_threshold": TOXIC_FLOW_THRESHOLD,
        "toxic_momentum_bps": TOXIC_MOMENTUM_BPS,
        "toxic_microprice_bps": TOXIC_MICROPRICE_BPS,
        ...
        "toxic_cancel_bid": int(metrics[12]),
        "toxic_cancel_ask": int(metrics[13]),
        ...
        "toxic_gate_bid": int(metrics[20]),
        "toxic_gate_ask": int(metrics[21]),
...
def render_console_summary(summary: dict) -> str:
    return "\n".join(
        [
            "",
            "========== BitMEX toxic-flow filtered maker 回测 ==========",
            f"日期: {summary['date']}",
            ...
            f"toxic gate: bid={summary['toxic_gate_bid']}, ask={summary['toxic_gate_ask']}",
            f"toxic cancel: bid={summary['toxic_cancel_bid']}, ask={summary['toxic_cancel_ask']}",
...
def render_report(summary: dict) -> str:
    return f"""# BitMEX XBTUSDT Toxic-Flow Filtered Maker 回测报告
...
## Toxic Filter Activity

- toxic gate bid: `{summary['toxic_gate_bid']}`
- toxic gate ask: `{summary['toxic_gate_ask']}`
- toxic cancel bid: `{summary['toxic_cancel_bid']}`
- toxic cancel ask: `{summary['toxic_cancel_ask']}`
- suppress bid: `{summary['suppress_bid']}`
- suppress ask: `{summary['suppress_ask']}`
"""


def write_aggregate(summaries: list[dict]) -> Path:
    tag = RESULT_TAG or f"toxic_flow_filtered_{EXCHANGE_MODEL}"
    out = RESULT_DIR / f"bitmex_xbtusdt_toxic_flow_filtered_maker_{tag}.aggregate.csv"
    fields = [
        "date",
        "total_pnl_usdt",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest BitMEX XBTUSDT toxic-flow filtered maker.")
    ...
    parser.add_argument("--toxic-flow-threshold", type=float, default=TOXIC_FLOW_THRESHOLD)
    parser.add_argument("--toxic-momentum-bps", type=float, default=TOXIC_MOMENTUM_BPS)
    parser.add_argument("--toxic-microprice-bps", type=float, default=TOXIC_MICROPRICE_BPS)
...
def apply_args(args: argparse.Namespace) -> None:
    ...
    global TOXIC_FLOW_THRESHOLD, TOXIC_MOMENTUM_BPS, TOXIC_MICROPRICE_BPS
    ...
    TOXIC_FLOW_THRESHOLD = args.toxic_flow_threshold
    TOXIC_MOMENTUM_BPS = args.toxic_momentum_bps
    TOXIC_MICROPRICE_BPS = args.toxic_microprice_bps
