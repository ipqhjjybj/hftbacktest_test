...
FLOW_DECAY = 0.92
FLOW_MIN_QTY = 300.0
ALPHA_FLOW_WEIGHT = 0.55
ALPHA_MOMENTUM_WEIGHT = 0.25
ALPHA_MICROPRICE_WEIGHT = 0.20
ALPHA_MOMENTUM_NORM_BPS = 1.0
ALPHA_MICROPRICE_NORM_BPS = 0.8
ALPHA_ANCHOR_SKEW_BPS = 4.0
ALPHA_SIDE_SPREAD_SKEW_BPS = 2.0
ALPHA_LEVEL_CUT_THRESHOLD = 0.35
...
@njit
def alpha_score(flow_sig, momentum_bps, micro_bps):
    momentum = clamp(momentum_bps / max(ALPHA_MOMENTUM_NORM_BPS, 1e-9), -1.0, 1.0)
    micro = clamp(micro_bps / max(ALPHA_MICROPRICE_NORM_BPS, 1e-9), -1.0, 1.0)
    alpha = (
        ALPHA_FLOW_WEIGHT * flow_sig
        + ALPHA_MOMENTUM_WEIGHT * momentum
        + ALPHA_MICROPRICE_WEIGHT * micro
    )
    return clamp(alpha, -1.0, 1.0)


@njit
def active_levels_for_side(side, alpha, inv):
    ...
    # Inventory reduces layers on the side that increases the current inventory.
    if side > 0 and inv > 0:
        levels -= int(math.ceil(inv * (levels - 1)))
    elif side < 0 and inv < 0:
        levels -= int(math.ceil(-inv * (levels - 1)))

    # Alpha reduces layers on the side that is likely to be adversely selected.
    if alpha > ALPHA_LEVEL_CUT_THRESHOLD and side < 0:
        levels -= int(math.ceil(alpha * (LADDER_LEVELS - 1)))
    elif alpha < -ALPHA_LEVEL_CUT_THRESHOLD and side > 0:
        levels -= int(math.ceil(-alpha * (LADDER_LEVELS - 1)))
...
@njit
def should_quote_level(side, level_idx, pos, alpha):
    ...
    return level_idx < active_levels_for_side(side, alpha, inv)


@njit
def quote_price(side, level_idx, depth, pos, alpha, vol_bps):
    mid = current_mid(depth)
    inv = inventory_ratio(pos)
    ...
    # Positive alpha means expected up-move: move anchor up, tighten bids, widen asks.
    side_spread_skew = -alpha * ALPHA_SIDE_SPREAD_SKEW_BPS if side > 0 else alpha * ALPHA_SIDE_SPREAD_SKEW_BPS
    half_bps = max(half_bps + side_spread_skew, min_spacing_bps)

    anchor_shift_bps = alpha * ALPHA_ANCHOR_SKEW_BPS - inv * INVENTORY_SKEW_BPS_AT_SOFT_LIMIT
    anchor = fair_microprice(depth) * (1.0 + anchor_shift_bps / 10_000.0)
    if side > 0:
...
def run_strategy(hbt, recorder, metrics, end_close_ts):
    ...
    while hbt.elapse(ORDER_UPDATE_INTERVAL_NS) == 0:
        ...
        if depth.best_bid > 0 and depth.best_ask > 0:
            ...
            flow_sig = flow_score(flow)
            alpha = alpha_score(flow_sig, momentum_bps, micro_bps)
            metrics[32] += alpha
            metrics[33] += abs(alpha)
            metrics[34] += 1
            if alpha > 0:
                metrics[35] += 1
            elif alpha < 0:
            ...
            for idx in range(loop_levels):
                bid_ok = should_quote_level(1, idx, pos, alpha)
                bid_px = quote_price(1, idx, depth, pos, alpha, vol_bps)
                ...
                ask_ok = should_quote_level(-1, idx, pos, alpha)
                ask_px = quote_price(-1, idx, depth, pos, alpha, vol_bps)
...
def run_backtest(bitmex_npz: Path, yyyymmdd: str) -> Path:
    ...
    tag = RESULT_TAG or f"alpha_skew_ladder_{EXCHANGE_MODEL}"
    out = RESULT_DIR / f"bitmex_xbtusdt_alpha_skew_ladder_{tag}_{yyyymmdd}.npz"
...
def write_summary(result_npz: Path, yyyymmdd: str) -> dict:
    ...
    avg_alpha = metrics[32] / metrics[34] if metrics[34] > 0 else 0.0
    avg_abs_alpha = metrics[33] / metrics[34] if metrics[34] > 0 else 0.0
    summary = {
        "date": yyyymmdd,
        "symbol": BITMEX_SYMBOL,
        "strategy": "alpha_skew_ladder",
        ...
        "alpha_anchor_skew_bps": ALPHA_ANCHOR_SKEW_BPS,
        "alpha_side_spread_skew_bps": ALPHA_SIDE_SPREAD_SKEW_BPS,
        ...
        "alpha_positive_samples": int(metrics[35]),
        "alpha_negative_samples": int(metrics[36]),
...
def render_console_summary(summary: dict) -> str:
    return "\n".join(
        [
            ...
            f"avg alpha: {summary['avg_alpha']:,.4f}, avg abs alpha: {summary['avg_abs_alpha']:,.4f}",
...
def render_report(summary: dict) -> str:
    return f"""# BitMEX XBTUSDT Alpha-Skew Ladder 回测报告
...
- avg alpha: `{summary['avg_alpha']:,.6f}`
...
def write_aggregate(summaries: list[dict]) -> Path:
    tag = RESULT_TAG or f"alpha_skew_ladder_{EXCHANGE_MODEL}"
    out = RESULT_DIR / f"bitmex_xbtusdt_alpha_skew_ladder_{tag}.aggregate.csv"
    fields = [
        "date",
        "total_pnl_usdt",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backtest BitMEX XBTUSDT alpha-skew ladder maker.")
    ...
    parser.add_argument("--alpha-anchor-skew-bps", type=float, default=ALPHA_ANCHOR_SKEW_BPS)
    parser.add_argument("--alpha-side-spread-skew-bps", type=float, default=ALPHA_SIDE_SPREAD_SKEW_BPS)
    parser.add_argument("--alpha-level-cut-threshold", type=float, default=ALPHA_LEVEL_CUT_THRESHOLD)
...
def apply_args(args: argparse.Namespace) -> None:
    ...
    global ALPHA_ANCHOR_SKEW_BPS, ALPHA_SIDE_SPREAD_SKEW_BPS, ALPHA_LEVEL_CUT_THRESHOLD
    ...
    ALPHA_ANCHOR_SKEW_BPS = args.alpha_anchor_skew_bps
    ALPHA_SIDE_SPREAD_SKEW_BPS = args.alpha_side_spread_skew_bps
    ALPHA_LEVEL_CUT_THRESHOLD = args.alpha_level_cut_threshold
