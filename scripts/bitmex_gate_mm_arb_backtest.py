...
# Latency calibration from live log around 2026-05-18T02:12:33:
# - BitMEX trade_time -> local execution receipt was roughly 41ms.
# - Local execution receipt -> Gate hedge websocket write was sub-ms.
# - Gate hedge websocket write -> Gate create_time_ms was roughly 141ms.
# - Gate create_time_ms -> local usertrade receipt was roughly 35ms.
#
# hftbacktest's asset order response latency already models the exchange/report
# round trip, so BitMEX fill-report latency belongs in BITMEX_ORDER_RESPONSE_LATENCY_NS.
BITMEX_FILL_REPORT_DELAY_NS = 0
LOCAL_GATE_SEND_DELAY_NS = 100_000
...
GATE_ORDER_ENTRY_LATENCY_NS = 140_000_000
GATE_ORDER_RESPONSE_LATENCY_NS = 35_000_000
